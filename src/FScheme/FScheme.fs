...
let LaceSingle = function
    | (Function(_) as f) :: lists ->
        let lists' = List.map (function List(l) -> l | m -> malformed "lace-single" m) lists
        Map <| f :: List.map (fun l -> List([List.head l])) lists'
    | m -> malformed "lace-single" <| List(m)
