...
namespace Dynamo.Nodes
{
    ...
    public class CustomNode : dynRevitTransactionNodeWithOneOutput
    {
        public CustomNode()
        {
            ...
            InPortData.Add(new PortData("in", "The first port's description.", new ObjectType(typeof(object))));

            //Define some output ports an output port will be created for 
            //the port data object you add to the OutPortData collection
            OutPortData.Add(new PortData("out", "The output value.", new ObjectType(typeof(object))));
